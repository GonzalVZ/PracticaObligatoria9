 ...
 */
 ...
 public class Usuario {
     ...
     public int save() {
         ...
         try {
             Statement st = con.createStatement();
             ResultSet rs = st.executeQuery("SELECT * FROM usuario WHERE id=" + this.getId());
             if (rs.next()) {
                 // Si el usuario ya existía, lo modificamos
                 filasAfectadas = st.executeUpdate("UPDATE usuario SET nick='" + this.getNick() + "', email='" + this.getEmail() + "' WHERE id=" + this.getId());
             } else {
                 // Si el usuario no existía, lo añadimos
                 filasAfectadas = st.executeUpdate("INSERT INTO usuario VALUES (" + this.getId() + ", '" + this.getNick() + "', '" + this.getEmail() + "')");
             }
             con.close();
         } catch (Exception e) {
             System.out.println("Error de SQL: " + e.getMessage());
     ...
     public int delete() {
         ...
         try {
             Statement st = con.createStatement();
             filasAfectadas = st.executeUpdate("DELETE FROM usuario WHERE id=" + this.getId());
             con.close();
         } catch (Exception e) {
             System.out.println("Error de SQL: " + e.getMessage());
         }
     ...
     private static Connection conectarBD() {
         ...
         try {
             ...
             con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/Usuario", "root", "root");         
            } catch (Exception e) {
             System.out.println("Error de SQL: " + e.getMessage());
         }
         return con;
